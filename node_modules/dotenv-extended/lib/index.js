...
var config = exports.config = function config(options) {
    ...
    if (options.errorOnMissing || options.errorOnExtra || options.errorOnRegex) {
        ...
        var _config = options.includeProcessEnv ? _lodash2.default.assign({}, configData, process.env) : configData;
        var schemaKeys = _lodash2.default.keys(schema);
        var configKeys = _lodash2.default.keys(_config);
        ...
        if (options.errorOnRegex) {
            var regexMismatchKeys = _lodash2.default.filter(schemaKeys, function (key) {
                if (schema[key]) {
                    return !new RegExp(schema[key]).test(_config[key]);
                }
            });
